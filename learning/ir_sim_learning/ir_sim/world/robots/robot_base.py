...
class RobotBase:
    ...
    def __init__(self, id, state, vel, goal=np.zeros(goal_dim), step_time=0.1, vel_min=[-inf, -inf], vel_max=[inf, inf],
                 acce=[inf, inf], angle_range=[-pi, pi], **kwargs):
    ...
    def arrive(self):
        if self.arrive_mode == 'position':
            if np.linalg.norm(
                    self.state[0:self.position_dim[0]] - self.init_goal_state[0:self.position_dim[0]]) <= self.goal_threshold:
                if not self.arrive_flag: logging.info('robot %d arrive at the goal', self.id)
                self.arrive_flag = True  # changed by me: True when initial goal is reached

        elif self.arrive_mode == 'state':
            if np.linalg.norm(self.state[0:self.goal_dim[0]] - self.init_goal_state) <= self.goal_threshold:
    ...
    def collision_check_map(self, state, pre_state):
        ...
        if collision_flag:
            ...
            distance_list = [RobotBase.distance(state[0:2], map_obstacle_positions[0:2, p:p + 1]) for p in
                             collision_index[0]]
    ...
    def get_info(self):
        ...
        return robot_info(self.state, self.vel, self.shape, self.G, self.h, self.cone_type, self.vel_min, self.vel_max,
                          self.acce)
    ...
    def plot_robot(self, ax, robot_color='g', goal_color='r', show_goal=True, show_text=False, show_traj=False,
                   traj_type='-g', fontsize=10, **kwargs):
