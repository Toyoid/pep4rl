...
class RobotOmni(RobotBase):
    ...
    def plot_robot(self, ax, robot_color = 'g', goal_color='r',
                   ...
                   traj_type='-c', fontsize=10, **kwargs):
        x = self.state[0, 0]
        y = self.state[1, 0]
        ...
        if show_goal:
            ...
            init_goal_circle = mpl.patches.Circle(xy=(init_goal_x, init_goal_y), radius=self.radius, color=goal_color,
                                                  alpha=0.5)
            init_goal_circle.set_zorder(1)
            goal_circle = mpl.patches.Circle(xy=(goal_x, goal_y), radius = (self.radius * 0.8), color='gold', alpha=1)
