...
class RobotDiff(RobotBase):
    ...
    def plot_robot(self, ax, robot_color='g', goal_color='r',
                    ...
                    show_uncertainty=False, traj_type='-c', fontsize=10,
                    arrow_width=0.4, arrow_length=0.3, **kwargs):
        x = self.state[0, 0]
        y = self.state[1, 0]
        
        goal_x = self.goal[0, 0]
        goal_y = self.goal[1, 0]
        init_goal_x = self.init_goal_state[0, 0]
        init_goal_y = self.init_goal_state[1, 0]
        ...
        arrow = mpl.patches.Arrow(x, y, arrow_length*cos(theta), arrow_length*sin(theta), width=arrow_width, color=robot_color)
        arrow.set_zorder(4)
        ax.add_patch(arrow)
        self.plot_patch_list.append(arrow)

        if show_goal:
            # add initial goal plot
            init_goal_circle = mpl.patches.Circle(xy=(init_goal_x, init_goal_y), radius=self.radius, color=goal_color, alpha=0.5)
            init_goal_circle.set_zorder(1)
            goal_circle = mpl.patches.Circle(xy=(goal_x, goal_y), radius = (self.radius * 0.8), color='orange', alpha=0.9)
            goal_circle.set_zorder(1)
        
            ax.add_patch(goal_circle)
            ax.add_patch(init_goal_circle)
            if show_text:
            ...
            self.plot_patch_list.append(init_goal_circle)
