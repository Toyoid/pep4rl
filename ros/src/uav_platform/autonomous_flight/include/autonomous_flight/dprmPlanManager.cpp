...
namespace AutoFlight{
    dprmPlanManager::dprmPlanManager(const ros::NodeHandle& nh) : flightBase(nh){
		this->initParam();
		this->initModules();
		this->updatePRMTimer_ = this->nh_.createTimer(ros::Duration(1.0), &dprmPlanManager::updatePRMTimerCB, this);
		this->resetRoadmapServer_ = this->nh_.advertiseService("/dep/reset_roadmap", &dprmPlanManager::resetRoadmapServiceCB, this);
	...
	void dprmPlanManager::initModules(){
		...
		this->expPlanner_.reset(new globalPlanner::DPRM (this->nh_));
	...
	void dprmPlanManager::updatePRMTimerCB(const ros::TimerEvent&) {
		if (this->expPlanner_->isUltimateTargetReceived()) {
			this->expPlanner_->setMap(this->map_);
			bool replanSuccess = this->expPlanner_->makePlan();
			this->expPlanner_->setCurrGoalRecieved(replanSuccess);
...
//		cout << "\033[1;32m[AutoFlight]: Takeoff succeed. Then PRESS ENTER to continue or PRESS CTRL+C to land.\033[0m" << endl;
	...
	// void dprmPlanManager::exploreReplan(){
	// 	ros::Rate r (1);
	// 	while (ros::ok()){
	// 		this->expPlanner_->setMap(this->map_);
